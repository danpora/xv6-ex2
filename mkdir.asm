...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3cf:	f7 f6                	div    %esi
 3d1:	0f b6 92 68 0c 00 00 	movzbl 0xc68(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 548:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 54b:	b8 5f 0c 00 00       	mov    $0xc5f,%eax
...
rand()
{
 ...
 7f0:	69 05 f4 10 00 00 0d 	imul   $0x19660d,0x10f4,%eax
 7f7:	66 19 00 
    if(uthread_pool[j].state != FREE && uthread_pool[j].state != SLEEP)
      current_ticket_num += uthread_pool[j].ntickets;
  }
...
rand()
{
 ...
 80c:	a3 f4 10 00 00       	mov    %eax,0x10f4
    if(uthread_pool[j].state != FREE && uthread_pool[j].state != SLEEP)
      current_ticket_num += uthread_pool[j].ntickets;
  }
...
{
  randstate = randstate * 1664525 + 1013904223;
 8f0:	69 05 f4 10 00 00 0d 	imul   $0x19660d,0x10f4,%eax
 8f7:	66 19 00 
//***************************************************************************************//
...
rand()
{
 ...
 903:	a3 f4 10 00 00       	mov    %eax,0x10f4
  return randstate;
}
 908:	c3                   	ret    
